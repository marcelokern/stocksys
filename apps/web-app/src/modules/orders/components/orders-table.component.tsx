...
import { ListOrderType } from "../types/orders.types";
...
const OrdersTable = ({
    ...
    handleView,
    handleUpdateStatus
}: TableComponentPropsType<ListOrderType>) => {
        ...
        contentLoader ? (
            ...
            data.length === 0 ? <div className="w-full text-center p-8 opacity-30">Nenhum pedido encontrado</div> : (
                ...
                <Table className="mt-8">
                    ...
                    <TableBody>
                        {data.map((x: ListOrderType, i: number) =>
                            <TableRow key={i}>
                                <TableCell># {x.code}</TableCell>
                                <TableCell>{new Date(x.date).toLocaleDateString('pt-BR')}</TableCell>
                                ...
                                <TableCell>
                                    ...
                                    {x.status === 'COMPLETE' && <Badge className="bg-green-500 hover:bg-green-500"><Check className="w-4 mr-2" />Finalizado</Badge>}
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent className="mr-6">
                                            ...
                                            <DropdownMenuItem onClick={() => handleView && handleView(x.id)}>
                                                <ListCollapse className="w-4 h-4 mr-2" />Ver pedido
                                            </DropdownMenuItem>

                                            {x.status === 'PENDING' && (

                                                <>
                                                    <DropdownMenuItem onClick={() => handleUpdateStatus && handleUpdateStatus(x.id, 'COMPLETE')}>
